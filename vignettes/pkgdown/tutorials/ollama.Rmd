...
Now that we have set up the Ollama model, we can use it with the `quallmer` package for qualitative coding tasks. The `quallmer` package uses openai's `gpt-4o` model by default, but we can easily switch to the Ollama model by specifying the `chat_fn` and `model` parameters in the `annotate()` function. For example, for Ollama models we would use `chat_ollama` as the `chat_fn` and specify the model name (e.g., `llama3.2:1b`) in the `model` parameter.
