...
The `qlm_code()` function with the built-in `task_sentiment()` codebook allows you to rate the sentiment of texts using an LLM. The predefined `task_sentiment()` codebook structures the response with a numeric sentiment score from -1 (very negative) to 1 (very positive) and a brief explanation. 
...
### Using `qlm_code()` for predefined sentiment analysis of texts

```{r sentiment}
# Apply predefined sentiment codebook with task_sentiment() in the qlm_code() function
result <- qlm_code(texts, codebook = task_sentiment(),
                   model = "openai/gpt-4o",
                   temperature = 0)
```
...
You can customize the sentiment analysis task by defining your own codebook with `qlm_codebook()` (for a more detailed explanation, [see our "Defining custom tasks" tutorial](https://seraphinem.github.io/quallmer/articles/pkgdown/tutorials/customtask.html)).
