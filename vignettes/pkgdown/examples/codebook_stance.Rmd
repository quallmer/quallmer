...
# Apply built-in stance codebook with codebook_stance() in the qlm_code() function
result <- qlm_code(data_corpus_inaugural, codebook = codebook_stance(topic),
                   model = "openai/gpt-4o",
                   temperature = 0)
```
...
You can customize the stance detection task by defining your own codebook with `qlm_codebook()` (for a more detailed explanation, [see our "Defining custom codebooks" tutorial](https://seraphinem.github.io/quallmer/articles/pkgdown/tutorials/custom-codebook.html)).
